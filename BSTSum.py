...
def bst_numbers_sum(root, num=0):
  return bst_numbers_sum_helper("", root)
  
def bst_numbers_sum_helper(prefix, root):
    if root is None:
        return 0
    if root.left is None and root.right is None:
        return int(prefix + str(root.value))
    left = bst_numbers_sum_helper(prefix + str(root.value), root.left)
    right = bst_numbers_sum_helper(prefix + str(root.value), root.right)
